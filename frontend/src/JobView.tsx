...
import type { FrameManifestItem, JobStatus, JobStatusValue, Segment } from './types'

const POLL_MS = 2000
const TERMINAL: JobStatusValue[] = ['ready', 'rendered', 'failed']
const SHOWABLE: JobStatusValue[] = ['ready', 'rendered', 'rendering']
...
export default function JobView({ jobId, onReset }: JobViewProps) {
  ...
  const showable = job?.status !== undefined && SHOWABLE.includes(job.status)
  useEffect(() => {
    if (!showable) return
    let cancelled = false
  ...
  if (loadError && !job)
    return (
      <div className="app">
        <p style={{ color: '#ff8f8f' }}>{loadError}</p>
      </div>
    )
  if (!job)
    return (
  ...
  return (
    <div className="app">
      <h1>
        cutlass<span>.</span> <span className="status">{job.status}</span>
      </h1>
      {busy && (
        <p className="muted">
      ...
      {job.status === 'failed' && <p style={{ color: '#ff8f8f' }}>Analysis failed: {job.error}</p>}
              ...
              <ul className="segments edit">
                {draft.map((s, i) => (
                  <li key={i} className="edit-row">
                    ...
                    <button className="icon" onClick={() => removeDraft(i)}>
                      ✕
                    </button>
                  </li>
                ))}
      ...
      <p>
        <button onClick={onReset} className="secondary" style={{ marginTop: 16 }}>
          New video
        </button>
      </p>
    </div>
